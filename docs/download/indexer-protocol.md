...
| `search.inputs` / `keywordsfilters` | Yes | Template subset: `.Keywords`, `.Categories`, `.Config.<key>`, `.Query.<field>`, `join`; block constructs `{{ if }}/{{ else }}/{{ end }}` (conditions: atoms, variadic `and`/`or`, binary `eq`/`ne`, parentheses, quoted literals, `.True`/`.False`) and `{{ range .Categories }}…{{ . }}…{{ end }}`; `$raw` input supported. Filter args render the same subset. Pipelines, `with`, `not`, `else if`, and templated `method:` are rejected at load |
| `search.rows` | `selector`, `remove`, `filters` | Row filters: `andmatch` — keeps only rows whose title carries every significant query keyword (case- and diacritic-insensitive; `and`/`the`/`an` and single characters ignored; optional arg truncates the keyword source), skipped when the search is by an ID the definition natively advertises. Unknown rows filters rejected at load. `after`, `dateheaders` warned and ignored |
| `search.fields` | Yes | `selector`, `attribute`, `text`, `default` (fallback template consulted when extraction is blank), `optional`, `case`, `remove`, filters. Fields evaluate in YAML declaration order; `text`, `default`, `case` values, and filter args render the template subset in row scope: `.Result.<field>` reads the fields extracted earlier in the same row (missing/empty reads as false, so `{{ if .Result.x }}` and `{{ or .Result.a .Result.b }}` coalesce as upstream) |
| Filter chains | Common set | `regexp`, `re_replace`, `replace`, `split`, `trim`, `prepend`, `append`, `tolower`, `toupper`, `querystring`, `dateparse` (alias `timeparse`), `timeago` (alias `reltime`, best-effort), `urldecode`, `urlencode`, `validfilename`, `validate`, `diacritics`; row-level `andmatch`. Unknown filters are rejected at load — the `fuzzytime` / `htmldecode` tail is not implemented yet |
...
| `settings` | Yes | Per-indexer overrides stored on the indexer row (`settings_json`), validated at client construction against the declared `settings:` fields, and overlaid onto `.Config.<key>` (definition `default:` → user override → injected `cookie`). `checkbox` values render as the literal strings `"true"`/`"false"`, which `{{ if .Config.x }}` conditions read as booleans |
| `ratio` | No | Deferred |
