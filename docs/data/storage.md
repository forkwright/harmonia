...
**No `BEGIN IMMEDIATE` for read transactions.** `BEGIN IMMEDIATE` acquires the write lock before any statements — unnecessary for reads and defeats WAL's concurrency advantage.
