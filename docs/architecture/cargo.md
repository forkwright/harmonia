...
graph TD
    ...
    %% Quality and supplements (must precede Syntaxis and Kathodos in graph)
    Kritike --> HC
    Kritike --> HDB
    Kritike --> Horismos
    ...
    %% Queue pipeline — depends on Ergasia and Kathodos
    Syntaxis --> HC
    Syntaxis --> Ergasia
    Syntaxis --> HDB
    Syntaxis --> Kathodos
...
Feature flags gate optional integrations. All feature flags are declared in `archon/Cargo.toml`; they are workspace-level integration switches, not per-library toggles.
